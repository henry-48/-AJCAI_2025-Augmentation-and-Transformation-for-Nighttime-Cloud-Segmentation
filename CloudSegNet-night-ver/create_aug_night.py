...
import scipy.misc
from tensorlayer.prepro import apply_transform
import imageio
...
def get_image_and_mask(img_id):
    ...
    img = tf.keras.utils.load_img(my_image,
                         target_size=(input_size, input_size))
    img = tf.keras.utils.img_to_array(img)
    mask = tf.keras.utils.load_img(my_GT,
...
for img_id in img_ids:
    ...
    for i in range(NO_OF_Xs):
        ...
        imageio.imwrite(data_dir + 'aug_SWINSEG/images/'+img_id+'_'+str(i)+'.jpg', img_aug)
        imageio.imwrite(data_dir + 'aug_SWINSEG/GTmaps/'+img_id+'_'+str(i)+'.jpg', mask_aug[:, :, 0])